...
Error ZabbixTrapper::sending(std::string value, Error *checkSending) {
	...
	asio::write(socket, asio::buffer(createZabbixPacket(value), _payload_size + kHeaderSize), error);
...
std::vector<char> ZabbixTrapper::createZabbixPacket(const std::string& value) {
	...
	if (_payload_size > kMinMessageSize) {
		msg.resize(_payload_size + kHeaderSize);
	}

	memset(msg.data(), 0x00, _payload_size + kHeaderSize);

	memcpy(msg.data(), kProtocolHeader.data(), kProtocolHeader.size());
	msg[kVersionOffset] = 0x01; // version
	memcpy(&msg[kLengthOffset], &_payload_size, sizeof(_payload_size));
	memcpy(&msg[kHeaderSize], payload.c_str(), _payload_size);
	return msg;
}

Error ZabbixTrapper::asyncCheckControl(unsigned int number_message) {
